...
The ContactForm API is a RESTful web service designed to handle contact form submissions, providing email delivery functionality with support for multiple SMTP configurations, templates, and attachments. This API is built using .NET 8 and follows a API approach to deploy in AWS Lambda Function.
...
Each email sent through the API is assigned a unique 8-character alphanumeric ID (uppercase) that serves multiple purposes:

- **Email Subject**: The ID is automatically appended to the email subject in the format `Subject - [ID]`
- **API Response**: The ID is returned in the API response for client-side tracking
- **Logging**: All logs and console output include the email ID for easy traceability
- **Consistency**: The same ID is maintained across SMTP failover attempts, ensuring a single email can be tracked even if multiple SMTP servers are tried

Example email subject: `Message from John Doe - [A3F2B1C9]`
...
| EmailId         | string  | No       | Unique email ID (auto-generated by service, read-only) |
...
- `message`: Success message with SMTP configuration details
- `emailId`: Unique 8-character identifier for the sent email

**Note:** The email ID is also automatically added to the email subject in the format `Subject - [emailId]` and appears in all application logs.
...
```json
{
  "message": "Test Email sent successfully using SMTP_1 (sender@example.com -> recipient@example.com)",
  "emailId": "B4C3D2E1"
}
```

The response format is identical to the Send Email endpoint, including the unique email ID.
